...
int terulet(int a_oldal, int b_oldal){
    return a_oldal*b_oldal;
}

int maximum(int szam[], int hossz){
    int leg_n = szam[0];
...
int maximum_index(int szam[], int hossz){
    int leg_n_index = szam[0];
    int leg_n = szam[0];;
    for (int i = 0; i < hossz; ++i) {
        if(szam[i] > leg_n){
            leg_n_index = i;
...
void rendez(LEGO_lap LEGO_lapok[], int n) {
    int legok_szama = 0;
    int teruletek[100];
    int sor_teruletek[100];
    LEGO_lap ujsorrend[n];
    for (int i = 0; i < n; ++i) {
        teruletek[i] = terulet(LEGO_lapok[i].a_oldal, LEGO_lapok[i].b_oldal);
    }

    while(legok_szama != n){
        sor_teruletek[legok_szama]  = maximum(teruletek, n);
        teruletek[maximum_index(teruletek, n)] = 0;
        legok_szama++;
    }
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            if(terulet(LEGO_lapok[i].a_oldal, LEGO_lapok[i].b_oldal) == sor_teruletek[j]){
                ujsorrend[i] = LEGO_lapok[i];
            }
        }
